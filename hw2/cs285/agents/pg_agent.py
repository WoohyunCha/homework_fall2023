...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        # beyond this point.
        obs, actions, rewards, terminals, q_values = np.concatenate(obs, axis=0), np.concatenate(actions, axis=0), np.concatenate(rewards, axis=0), np.concatenate(terminals, axis=0), np.concatenate(q_values, axis=0)
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [self._discounted_return(rewards=rew) for rew in rewards]
        else:
            # Case 2: in reward-to-go PG, we only use the rewards after timestep t to estimate the Q-value for (s_t, a_t).
            # In other words: Q(s_t, a_t) = sum_{t'=t}^T gamma^(t'-t) * r_{t'}
            # use the helper function self._discounted_reward_to_go to calculate the Q-values
            q_values = [self._discounted_reward_to_go(rewards=rew) for rew in rewards]
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    if terminals[i]:
                        ...
                        advantages[i] = advantages[i+1] * (self.gamma*self.gae_lambda) + (rewards[i] + values[i] - values[i+1])
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for t, rew in enumerate(rewards):
            ret += (self.gamma**t)*rew
        return [ret for _ in range(len(rewards))]
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for t in range(len(rewards)):
            sum = 0
            for i in np.arange(t, len(rewards)):
